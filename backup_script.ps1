...
Write-Host "=== GitHub Repository Backup to S3 Started ===" -ForegroundColor Green
...
try {
    ...
    try {
        ...
        Write-Host "   App ID: $githubAppId" -ForegroundColor Yellow
        Write-Host "   Error: $($_.Exception.Message)" -ForegroundColor Red
        exit 1
    }
    ...
    try {
        ...
        Write-Host "   - GitHub App is not installed on any repositories" -ForegroundColor Yellow
        Write-Host "   - App ID or private key is incorrect" -ForegroundColor Yellow
        Write-Host "   - App permissions are insufficient" -ForegroundColor Yellow
        Write-Host "   Error: $($_.Exception.Message)" -ForegroundColor Red
        exit 1
    }
...
function Get-GitHubInstallationToken {
    ...
    $headers = @{
        ...
        "User-Agent" = "PowerShellGitHubBackup/1.0"
    }
    ...
    try {
        ...
        foreach ($repo in $repositories) {
            ...
            try {
                ...
                Write-S3Object -BucketName $S3Bucket -File $localZip -Key $s3Key -Region $AwsRegion -Force
